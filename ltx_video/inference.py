...
def infer(config: InferenceConfig):
    ...
    logger.warning("=" * 60)
    logger.warning("LTX-Video  —  starting inference")
    logger.warning(f"  Prompt          : {config.prompt}")
    logger.warning(f"  Negative prompt : {config.negative_prompt}")
    logger.warning(f"  Resolution      : {config.width}x{config.height}")
    logger.warning(f"  Frames / FPS    : {config.num_frames} frames @ {config.frame_rate} fps")
    logger.warning(f"  Seed            : {config.seed}")
    logger.warning(f"  Pipeline config : {config.pipeline_config}")
    logger.warning("=" * 60)
    ...
    if not os.path.isfile(ltxv_model_name_or_path):
        logger.warning(f"[1/7] Downloading model checkpoint: {ltxv_model_name_or_path}")
        ltxv_model_path = hf_hub_download(
            repo_id="Lightricks/LTX-Video",
            filename=ltxv_model_name_or_path,
            repo_type="model",
        )
        logger.warning(f"      Checkpoint saved to: {ltxv_model_path}")
    else:
        logger.warning(f"[1/7] Using local checkpoint: {ltxv_model_name_or_path}")
    ...
    if spatial_upscaler_model_name_or_path and not os.path.isfile(
        ...
        logger.warning(f"      Downloading spatial upscaler: {spatial_upscaler_model_name_or_path}")
    ...
    # ── 2. Device / memory setup ──────────────────────────────────────────────
    device = get_device()
    gpu_mem = get_total_gpu_memory()
    logger.warning(f"[2/7] Device: {device}" + (f"  (VRAM: {gpu_mem:.1f} GB)" if gpu_mem else ""))
    ...
    if config.offload_to_cpu and not torch.cuda.is_available():
        ...
        offload_to_cpu = config.offload_to_cpu and gpu_mem < 30
        if offload_to_cpu:
            logger.warning(f"      CPU offloading enabled (VRAM {gpu_mem:.1f} GB < 30 GB threshold).")
    ...
    if height_padded != config.height or width_padded != config.width or num_frames_padded != config.num_frames:
        logger.warning(
            f"      Padded to {width_padded}x{height_padded}x{num_frames_padded} "
            f"(requested {config.width}x{config.height}x{config.num_frames})"
        )

    # ── 3. Prompt enhancement decision ───────────────────────────────────────
    ...
    if enhance_prompt:
        logger.warning(
            f"[3/7] Prompt enhancement enabled "
            f"({prompt_word_count} words < threshold {prompt_enhancement_words_threshold})."
        )
    else:
        if prompt_enhancement_words_threshold > 0:
            logger.warning(
                f"[3/7] Prompt enhancement disabled "
                f"({prompt_word_count} words >= threshold {prompt_enhancement_words_threshold})."
            )
        else:
            logger.warning("[3/7] Prompt enhancement disabled (threshold = 0).")
    ...
    # ── 4. Pipeline load / cache ──────────────────────────────────────────────
    logger.warning(f"[4/7] Loading pipeline  (precision={precision}, sampler={sampler}) …")
    t0 = time.time()
    ...
    logger.warning(f"      Pipeline ready  ({time.time() - t0:.1f}s)")
    ...
    if pipeline_config.get("pipeline_type", None) == "multi-scale":
        ...
        logger.warning("      Building multi-scale pipeline with latent upsampler …")
        latent_upsampler = create_latent_upsampler(
            spatial_upscaler_model_path, pipeline.device
        )
        pipeline = LTXMultiScalePipeline(pipeline, latent_upsampler=latent_upsampler)

    # ── 5. Load input / conditioning media ───────────────────────────────────
    media_item = None
    if config.input_media_path:
        logger.warning(f"[5/7] Loading input media: {config.input_media_path}")
        t0 = time.time()
        ...
        logger.warning(f"      Input media loaded  ({time.time() - t0:.1f}s)")
    else:
        logger.warning("[5/7] No input media (text-to-video mode).")

    if conditioning_media_paths:
        logger.warning(
            f"      Loading {len(conditioning_media_paths)} conditioning item(s): "
            + ", ".join(conditioning_media_paths)
        )
        t0 = time.time()
    ...
    if conditioning_media_paths:
        logger.warning(f"      Conditioning items ready  ({time.time() - t0:.1f}s)")
    ...
    logger.warning(f"      STG mode: {stg_mode}")

    # ── 6. Run the diffusion pipeline ────────────────────────────────────────
    logger.warning("[6/7] Running diffusion pipeline …")
    t0 = time.time()
    ...
    logger.warning(f"      Diffusion complete  ({time.time() - t0:.1f}s)")

    # ── 7. Post-process and save ──────────────────────────────────────────────
    logger.warning("[7/7] Post-processing and saving output …")
    t0 = time.time()
    ...
    for i in range(images.shape[0]):
        ...
        # Cast to float32 on the GPU before moving to CPU — much faster than casting on CPU.
        video_np = images[i].permute(1, 2, 3, 0).float().cpu().numpy()
        ...
        if video_np.shape[0] == 1:
            ...
            # Write video with PyAV directly — far faster than imageio's
            # frame-by-frame append, which serialises each frame individually.
            logger.warning(
                f"      Writing {video_np.shape[0]} frames "
                f"({width}x{height}) @ {fps} fps …"
            )
            with av.open(str(output_filename), "w", format="mp4") as container:
        ...
        logger.warning(f"      Saved: {output_filename}  ({time.time() - t0:.1f}s)")

    logger.warning(f"      Post-processing done  ({time.time() - t0:.1f}s)")
