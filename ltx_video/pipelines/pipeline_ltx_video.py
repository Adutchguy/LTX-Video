...
class LTXVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        logger.warning("  [pipeline] Encoding prompt with text encoder …")
        t0 = time.time()
        if self.text_encoder is not None:
        ...
        logger.warning(f"  [pipeline] Prompt encoded  ({time.time() - t0:.1f}s)")
        ...
        logger.warning("  [pipeline] Preparing initial latents …")
        t0 = time.time()
        ...
        logger.warning(
            f"  [pipeline] Latents ready  shape={list(latents.shape)}  ({time.time() - t0:.1f}s)"
        )
        ...
        logger.warning(
            f"  [pipeline] Denoising  {num_inference_steps} steps  "
            f"device={self._execution_device}"
        )
        _denoising_start = time.time()
        _step_times: list = []

        # Before compiling this code please be aware:
        # This code might generate different input shapes if some timesteps have no STG or CFG.
        # This means that the codes might need to be compiled multiple times.
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                logger.warning(
                    f"    step {i + 1:>2}/{num_inference_steps}  "
                    f"t={float(t):.4f}  "
                    f"({_step_elapsed:.2f}s)"
                )

                if callback_on_step_end is not None:
                    callback_on_step_end(self, i, t, {})

        _denoising_elapsed = time.time() - _denoising_start
        if _step_times:
            avg_step = sum(_step_times) / len(_step_times)
            logger.warning(
                f"  [pipeline] Denoising complete  "
                f"total={_denoising_elapsed:.1f}s  "
                f"avg/step={avg_step:.2f}s"
            )
        ...
        if output_type != "latent":
            logger.warning("  [pipeline] Decoding latents via VAE …")
            _t0_vae = time.time()
            ...
            logger.warning(f"  [pipeline] VAE decode complete  ({time.time() - _t0_vae:.1f}s)")
